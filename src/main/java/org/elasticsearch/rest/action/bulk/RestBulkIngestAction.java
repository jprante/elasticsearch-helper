...
import org.elasticsearch.action.bulk.BulkIngestProcessor;
import org.elasticsearch.action.bulk.BulkIngestRequest;
...
public class RestBulkIngestAction extends BaseRestHandler {

    private final static ESLogger logger = Loggers.getLogger(RestBulkIngestAction.class);
    /**
     ...
     * The IngestProcessor
     */
    private final BulkIngestProcessor ingestProcessor;

    @Inject
    public RestBulkIngestAction(Settings settings, Client client, RestController controller) {
        super(settings, client);

        controller.registerHandler(POST, "/_bulkingest", this);
        controller.registerHandler(PUT, "/_bulkingest", this);
        controller.registerHandler(POST, "/{index}/_bulkingest", this);
        controller.registerHandler(PUT, "/{index}/_bulkingest", this);
        controller.registerHandler(POST, "/{index}/{type}/_bulkingest", this);
        controller.registerHandler(PUT, "/{index}/{type}/_bulkingest", this);

        int actions = settings.getAsInt("action.bulkingest.maxactions", 100);
        int concurrency = settings.getAsInt("action.bulkingest.maxconcurrency", 30);
        ByteSizeValue volume = settings.getAsBytesSize("action.bulkingest.maxvolume", ByteSizeValue.parseBytesSizeValue("5m"));

        this.ingestProcessor = BulkIngestProcessor.builder(client)
                .actions(actions)
    ...
    public void handleRequest(final RestRequest request, final RestChannel channel) {
        ...
        BulkIngestProcessor.Listener listener = new BulkIngestProcessor.Listener() {
            @Override
            public void beforeBulk(long executionId, BulkIngestRequest ingestRequest) {
